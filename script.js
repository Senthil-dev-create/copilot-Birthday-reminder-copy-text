...
document.getElementById('copyBirthdayRecordsBtn').addEventListener('click', () => {
    const remindersTable = document.getElementById('remindersTable');
    const range = document.createRange();
    range.selectNode(remindersTable);
    window.getSelection().removeAllRanges(); // clear current selection
    window.getSelection().addRange(range); // to select text
    try {
        document.execCommand('copy');
        showFeedback('Records copied to clipboard!', 'success', 'feedbackMessageView');
    } catch (err) {
        showFeedback('Failed to copy!', 'danger', 'feedbackMessageView');
    }
